...
namespace Invoicer.Services.Impl
{
    public partial class PdfInvoice
    {
        private void HeaderSection()
        {
            ...
            row = subTable.AddRow();
            row.Cells[0].AddParagraph("Fatura No:", ParagraphAlignment.Left, "H2-9B-Color");
            row.Cells[1].AddParagraph(Invoice.Reference, ParagraphAlignment.Right, "H2-9");
            row.Cells[0].AddParagraph("Fatura Tarihi:", ParagraphAlignment.Left, "H2-9B-Color");
            row.Cells[1].AddParagraph(Invoice.BillingDate.ToShortDateString(), ParagraphAlignment.Right, "H2-9");
            row.Cells[0].AddParagraph("Ödeme Tarihi:", ParagraphAlignment.Left, "H2-9B-Color");
        ...
        private void BillingHeader(Table table)
        {
            ...
            row.Cells[0].AddParagraph("AÇIKLAMA", ParagraphAlignment.Left);
            row.Cells[1].AddParagraph("SAYI/ADET", ParagraphAlignment.Center);
            row.Cells[2].AddParagraph("VERGİ %", ParagraphAlignment.Center);
            row.Cells[3].AddParagraph("BİRİM TUTAR", ParagraphAlignment.Center);
            if (Invoice.HasDiscount)
            {
                row.Cells[4].AddParagraph("İNDİRİM", ParagraphAlignment.Center);
                row.Cells[5].AddParagraph("TOPLAM", ParagraphAlignment.Center);
            }
            else
            {
                row.Cells[4].AddParagraph("TOPLAM", ParagraphAlignment.Center);
            }
        }
        ...
        private void BillingRow(Table table, ItemRow item)
        {
            ...
            cell.AddParagraph(item.Aciklama, ParagraphAlignment.Left, "H2-9-Grey");
        ...
        private void PaymentSection()
        {
            ...
            if (Invoice.Company.HasCompanyNumber || Invoice.Company.HasVatNumber)
            {
                ...
                if (Invoice.Company.HasCompanyNumber && Invoice.Company.HasVatNumber)
                {
                    row.Cells[0].AddParagraph(string.Format("ŞİRKET NO: {0}, ŞİRKET VERGİ: {1}",
                ...
                else
                {
                    if (Invoice.Company.HasCompanyNumber)
                        row.Cells[0].AddParagraph(string.Format("ŞİRKET VERGİ: {0}", Invoice.Company.CompanyNumber),
                        ParagraphAlignment.Center, "H2-9B-Inverse")
                        .Format.Shading.Color = shading;
                    else
                        row.Cells[0].AddParagraph(string.Format("KDV No: {0}", Invoice.Company.VatNumber),
                        ParagraphAlignment.Center, "H2-9B-Inverse")
                        .Format.Shading.Color = shading;
                }
            }
